...
import 'package:flutterquiz/features/tender/cubits/subCategoryCubit.dart';
import 'package:flutterquiz/features/tender/cubits/unlockedLevelCubit.dart';
...
class TenderScreen extends StatefulWidget {
  ...
  final String level; //will be in use for tenderZone tenderType
  final String categoryId; //will be in use for tenderZone tenderType
  final String subcategoryId; //will be in use for tenderZone tenderType
  final String
      subcategoryMaxLevel; //will be in use for tenderZone tenderType (to pass in result screen)
  final int unlockedLevel;
  final bool isPlayed; //Only in use when quiz type is audio questions
  final String contestId;
  final Comprehension
      comprehension; // will be in use for fun n learn tenderType (to pass in result screen)
  ...
  static Route<dynamic> route(RouteSettings routeSettings) {
    ...
    //keys of arguments are numberOfPlayer and tenderType (required)
    //if tenderType is tenderZone then need to pass following keys
...
class _TenderScreenState extends State<TenderScreen> with TickerProviderStateMixin {
  ...
  late AnimationController audioTimerController = AnimationController(
    vsync: this,
    duration: Duration(
      seconds: widget.tenderType == TenderTypes.audioQuestions
  ...
  late final timerAnimationController = AnimationController(
    ...
    duration: Duration(
      seconds: widget.tenderType == TenderTypes.mathMania
          ? context.read<SystemConfigCubit>().getMathsTenderTime()
          : widget.tenderType == TenderTypes.funAndLearn
              ? context.read<SystemConfigCubit>().getFunAndLearnTime()
              : widget.tenderType == TenderTypes.audioQuestions
                  ? context.read<SystemConfigCubit>().getAudioTimer()
                  : context.read<SystemConfigCubit>().getTenderTime(),
    ),
  ...
  late double totalSecondsToCompleteTender = 0;
  ...
  void navigateToResultScreen() {
    ...
    Navigator.of(context).pushReplacementNamed(Routes.result, arguments: {
      ...
      "timeTakenToCompleteTender": totalSecondsToCompleteTender,
  ...
  Map<String, LifelineStatus> getLifeLines() {
    if (widget.tenderType == TenderTypes.tenderZone ||
        widget.tenderType == TenderTypes.dailyTender) {
      return lifelines;
    }
    return {};
  }

  void updateTotalSecondsToCompleteTender() {
    final configCubit = context.read<SystemConfigCubit>();
    totalSecondsToCompleteTender = totalSecondsToCompleteTender +
        UiUtils.timeTakenToSubmitAnswer(
          ...
          tenderZoneTimer: configCubit.getTenderTime(),
        );
  }
  ...
  void submitAnswer(String submittedAnswer) async {
    ...
    if (!context
      ...
      updateTotalSecondsToCompleteTender();
      ...
      if (currentQuestionIndex !=
        ...
        updateSubmittedAnswerForBookmark(
            context.read<QuestionsCubit>().questions()[currentQuestionIndex]);
        changeQuestion();
        //if tenderType is not audio or latex(math or chemistry) then start timer again
        if (widget.tenderType == TenderTypes.audioQuestions ||
            widget.tenderType == TenderTypes.mathMania) {
  ...
  Widget _buildBookmarkButton() {
    ...
    //if tender type is audio questions
    if (widget.tenderType == TenderTypes.audioQuestions) {
      print("audioQue");
      return BlocBuilder<QuestionsCubit, QuestionsState>(
        bloc: questionsCubit,
  ...
  void onTapBackButton() {
    ...
    showDialog(
      ...
      builder: (_) => ExitGameDialog(
        onTapYes: (widget.tenderType == TenderTypes.tenderZone)
            ? () {
  ...
  Widget _buildLifeLines() {
    if (widget.tenderType == TenderTypes.dailyTender ||
        widget.tenderType == TenderTypes.tenderZone) {
      return Container(
        alignment: Alignment.bottomCenter,
        padding:
          ...
          return WillPopScope(
            ...
            child: Scaffold(
              appBar: QAppBar(
                ...
                roundedAppBar: false,
                title: widget.tenderType == TenderTypes.funAndLearn
                    ? AnimatedBuilder(
